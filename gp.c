...
static uint32_t peri_base = 0u;
static uint32_t page_size = 0u;

static uint32_v *ugpio = 0;
static uint32_v *upads = 0;
...
int
gpio_set_alt_function(int gpio,IO io) {
	int shift;

	if ( gpio < 0 )
		return EINVAL;          // Invalid parameter
	set_gpio10(gpio,&shift,GPIO_GPFSEL0);
	return 0;
}
